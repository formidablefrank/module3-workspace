    ...
    <div id="wrapper">
      ...
      <div id="page-wrapper">
        ...
        <div class="row">
	      <div class="col-lg-4">
            <form action="cust">
               <select class="form-control" name="categoryName" onchange="this.form.submit()">
          ...
          <div class="col-lg-12">
            ...
            <div class="table-responsive">
              <table class="table table-hover table-bordered table-striped tablesorter">
                ...
                <tbody>
                  <c:forEach items="${inventory.getProductList().keySet()}" var="product">
                  	<tr>
                      <td>${product.getProductName()}</td>
                      <td>Php ${product.getPrice()}</td>
                      <td>${inventory.getProductList().get(product)}</td>
                      <td><img src="${product.getImageURL()}" width="50" height="50"></img></td>
                      <td>
                      	<div class = "row">
                      	  <div class="col-lg-6">
                      	<form action="addToCart" method="post">
                      	  	<select class="form-control" name="quantity" onchange="this.form.submit()">
                      	  	  <c:forEach begin="0" end="${inventory.getProductList().get(product)}" var="q">
                      	  	    <option>${q}</option>
                      	  	  </c:forEach>
                      	  	</select>
                      	    <input type="hidden" name="productName" value="${product.getProductName()}">
                      	    <input type="hidden" name="category" value="${category.getCategoryName()}">
                      	</form>
                      	  </div>
                      	</div>
                      </td>
