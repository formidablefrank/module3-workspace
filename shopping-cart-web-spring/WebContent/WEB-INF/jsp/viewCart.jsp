  ...
  <body>
    ...
    <div id="wrapper">
      ...
      <div id="page-wrapper">
          ...
          <div class="col-lg-12">
            <div class="table-responsive">
              <table class="table table-hover table-bordered table-striped tablesorter">
                ...
                <tbody>
                  <c:forEach items="${cart.getOrders()}" var="od">
                  	<tr>
                      <td>${od.product.getProductName()}</td>
                      <td>Php ${od.product.getPrice()}</td>
                      <td>${od.orderQuantity}</td>
                      <td>${od.product.getCategory().getCategoryName()}</td>
                      <td><img src="${od.product.getImageURL()}" width="50" height="50"></img></td>
                      <td>
                      	<div class = "row">
                      	  <div class="col-lg-4">
                      		<form action="removeFromCart" method="post">
                      	  	<select class="form-control" name="quantity" onchange="this.form.submit()">
                      	  	  <c:forEach begin="0" end="${od.orderQuantity}" var="q">
                      	  	    <option>${q}</option>
                      	  	  </c:forEach>
                      	  	</select>
                      	    <input type="hidden" name="productName" value="${od.product.getProductName()}">
                      		</form>
                      	  </div>
                      	</div>
                      </td>
        ...
        <c:if test="${!cart.getOrders().isEmpty()}">
        <div class="row">
          <div class="col-lg-6">
        ...
        <c:choose>
	       <c:when test="${cart.getOrders().isEmpty()}">
			 <div class="row">
        <div class="col-lg-6">
            <div class="alert alert-dismissable alert-info">
              <!--<button type="button" class="close" data-dismiss="alert">&times;</button>-->
